export const memberText = {
  point_style: "积分",
  balance_style: "消费"

};

export const distribute = {
  logo: "",
  content: "",
  distribution_label: "2",
  distribution_name: "分销中心",
  distributor_name: "分销商",
  distribution_commission: "分销佣金",
  total_commission: "已结佣金",
  commission: "佣金",
  commission_details: "佣金明细",
  withdrawable_commission: "可提佣金",
  withdrawals_commission: "已提佣金",
  withdrawal: "提现中",
  frozen_commission: "未结佣金",
  distribution_order: "分销订单",
  my_team: "我的团队",
  team1: "一级",
  team2: "二级",
  team3: "三级",
  my_customer: "渠道部门",
  extension_code: "推广码",
  distribution_tips:
    "分销小提示：可以通过二维码、邀请链接或者邀请码发展下线客户，下线客户购买商品后你可以获得相应的佣金奖励",
  become_distributor: "成为分销商"
};

export const bonus = {
  common: {
    bonus_name: "分红中心",
    bonus: "分红",
    withdrawals_bonus: "已发放分红",
    withdrawal_bonus: "待发放分红",
    frozen_bonus: "待结算",
    bonus_details: "分红明细",
    bonus_money: "分红金额",
    bonus_order: "分红订单"
  },
  global: {
    withdrawals_global_bonus: "已发放分红",
    withdrawal_global_bonus: "待发放分红",
    frozen_global_bonus: "待结算",
    apply_global: "申请全球股东",
    global_agreement: "全球股东"
  },
  area: {
    withdrawals_area_bonus: "已发放分红",
    withdrawal_area_bonus: "待发放分红",
    frozen_area_bonus: "待结算",
    apply_area: "申请区域代理",
    area_agreement: "区域代理"
  },
  team: {
    withdrawals_team_bonus: "已发放分红",
    withdrawal_team_bonus: "待发放分红",
    frozen_team_bonus: "待结算",
    apply_team: "申请团队队长",
    team_agreement: "团队队长"
  }
};

export const thingcircle = {
  thingcircle: "社交圈",
  ganhuo: "干货"
};
export const thingcircletwo = {
  thingcircletwo: "商务圈",
  ganhuo: "干货"
};