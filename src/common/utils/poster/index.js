    ...
    try {
      ...
      if (drawArray && drawArray.length > 0) {
        for (let i = 0; i < drawArray.length; i++) {
          const drawArrayItem = drawArray[i];
          _app.log(
            "绘制可控层级序列, drawArrayItem:" + JSON.stringify(drawArrayItem)
          );
          switch (drawArrayItem.type) {
            case "image":
...
function drawFillRect(Context, drawArrayItem = {}) {
  //填充矩形
  _app.log(
    "进入绘制填充直角矩形方法, drawArrayItem:" + JSON.stringify(drawArrayItem)
  );
...
function drawQrCode(Context, qrCodeObj) {
  ...
  let options = {
    ...
    correctLevel: Number(qrCodeObj.correctLevel || 0) || 1, // 容错级别
