<template>
  <view class="pages">
    ...
    <view class="foot-btn-group">
      <lk-button block round theme-color @click="reserveTechnician">
        保存
      </lk-button>
    </view>
    <view
    ...
    >
      <view class="cu-dialog" @tap.stop="">
        ...
        <view class="grid col-3 padding-sm">
          ...
          >
            <button
              ...
              :class="values == index ? 'bg-orange' : 'line-orange'"
              @tap="ChooseCheckbox"
              :data-value="index"
            >
              {{ item.name }}
        ...
        <view class="cu-bar bg-white">
          ...
          <view class="btn bg-blue" @tap="confirm"
            >确定({{ timeData[TabCur].year }}/{{ timeData[TabCur].date }}
            {{ checkbox[values].name }})</view
          >
        </view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    async confirm() {
      ...
      let date = new Date();
      let time =
        this.timeData[this.TabCur].year +
        '/' +
        this.timeData[this.TabCur].date +
        ' ' +
        this.checkbox[this.values].name;

      let selectedTime = new Date(time);
      let currentTimestemp = serverTime; // date.getTime()
    ...
    reserveList() {
      if (this.tec_id) {
        GET_RESERVELIST({ tec_id: this.tec_id })
          .then(({ data }) => {
