  ...
  <view class="pages">
    ...
    <view class="containar" v-if="canUseGetUserProfileThree">
      <view class="avatarUrl">
        ...
        <button class="attext" open-type="chooseAvatar" @chooseavatar="onChooseavatar">请上传头像</button>
      </view>
      <view class="nickname">
        <text>昵称：</text>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if(this.params.avatarUrl === '' || this.params.avatarUrl == this.oldavatarUrl){
        uni.showToast({
          title: "请上传头像",
          duration: 2000
        });
      ...
      this.authLogin(this.params).then(({ message }) => {
        this.$Prompt
            .toast({ icon: "success", title: message })
            .then(() => {
    ...
    getUserProfile() {
      ...
      Promise.all([p2,p1])
          .then(res => {
            ...
            this.authLogin(this.params).then(({ code,message }) => {
