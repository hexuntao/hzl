...
import { GET_GOODSCATEGORY,GET_GOODSCATEGORY_SHOP } from "@/common/interface/goods";
export default {
  ...
  onLoad() {},
  onShow() {
    GET_GOODSCATEGORY_SHOP({shop_id:this.routeQuery.shop_id})
      .then(({ data }) => {
        data.forEach(e => {
  ...
  methods: {
    click(e) {
      this.$Navigate.push({
        ...
        query: {
          type: "goods",
		  shop_id:this.routeQuery.shop_id
        }
      });
    },
    toList(id, name) {
