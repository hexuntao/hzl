...
import { DEL_THINGCIRCLETWO } from "@/common/interface/thingcircletwo";
// #ifdef APP-PLUS
...
export default {
  ...
  methods: {
    ...
    clickEdit() {
      this.$Navigate.push({
        path: "/packages/thingcircletwo/release/index",
        query: {
    ...
    clickRemove() {
      this.$Prompt
        ...
        .then(() => {
          DEL_THINGCIRCLETWO({ ids: this.info.id })
            .then(() => {
              this.$Prompt
                .toast({
                  title: "删除成功",
                ...
                .then(() => {
                  this.$Navigate.replace("/pages/thingcircletwo/index");
                });
            })
            .catch(() => {});
        })
