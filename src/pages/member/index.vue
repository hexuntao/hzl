  ...
  <view style="background-size: 100% 100%;background-repeat: no-repeat;width:750rpx;height:420rpx;" :style="{backgroundImage: 'url('+info.wap_member_index_adv+')'}">
      ...
      <block >
        ...
        <view class="info" >
          ...
          <view class="item btn-set" @tap="toLink('/packages/member/set')">
            <text class="text" style="padding:5rpx 25rpx;border:1rpx solid #fff;color:#fff;font-size:18rpx;border-radius: 20rpx;">设置</text>
          </view>
        ...
        <view class="btn-right" slot="right" style="marign-top:30rpx;">
          ...
          <view class="item btn-set" @tap="toLink('/packages/distribute/qrcode')" >
	...
	<view style="margin-top:20rpx;">
			<view  v-for="(item, index) in member_index_list" :key="index">
				<view style="border-bottom:1rpx solid #c7c7c7;margin:0 20rpx;height:auto;">
					<view  style="height:80rpx;" @click="showupmenu(index)">
						<view style="float:left;width:50%;color:#333;">
							<view style="margin-left:30rpx;line-height:80rpx;font-size:26rpx;">
								<image v-if="showindex!=index" style="vertical-align: middle;margin-top:-8rpx;width:36rpx;height:36rpx;margin-right:20rpx;" :src="item.category_pic1" ></image>
								<image v-if="showindex==index" style="vertical-align: middle;margin-top:-8rpx;width:36rpx;height:36rpx;margin-right:20rpx;" :src="item.category_pic" ></image>
								
								{{item.short_name}}
							</view>
						</view>
						<view   style="width:50%;float:right;text-align:right;padding-top:20rpx;" >
							<view style="margin-right:30rpx;">
								<lk-icon name="arrow" v-if="showindex!=index" color="#c7c7c7" />
								<lk-icon name="arrow-down" v-if="showindex==index" color="#c7c7c7" />
							</view>
						</view>
					...
					<view v-if="showindex==index">
						<view @click="toLink(item1.url)" v-for="(item1, index1) in item.child_list" :key="index1" style="float:left;text-align:center;padding:30rpx 0;" :style="'width:'+item.bili+'%'">
							<view><image style="width:50rpx;height:50rpx;" :src="item1.category_pic2" ></image></view>
							<view style="font-size:24rpx;margin-top:8rpx;">{{item1.short_name}}</view>
						</view>
