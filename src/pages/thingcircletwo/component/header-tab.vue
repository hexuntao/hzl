<template>
  <view class="header" :style="fixedTop" style="background-color:#fff;">
	...
	<view class="icon-tabs-group" v-if="is_show_search==0" style="position:relative;">
...
import { GET_TOPICLISTSECOND } from "@/common/interface/thingcircletwo";
  ...
  getData() {
    ...
    onChange(index) {
      if (index === 2) {
        ...
        this.$emit("change", { follow: index === 0 ? 1 : 0, lat: "", lng: "",type:'' });
      }
    },
	onChange2(index) {
	    ...
	    this.$emit("change", { follow: this.active === 0 ? 1 : 0, lat: "", lng: "",type:topic_title });
	  }
	},
	showSearch(){
